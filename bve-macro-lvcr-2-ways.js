...
const MACRO_VERSION = '1.0.230701';

const LVCR_ROLE_1 = 'lvcr-1';
const LVCR_ROLE_2 = 'lvcr-2';
const LVCR_ROLE_3 = 'lvcr-3';
const LVCR_ROLE_4 = 'lvcr-4';
const LVCR_POSITION_LEFT  = [2,1];
const LVCR_POSITION_RIGHT = [1,2];

const LVCR_ROLE_SELF = LVCR_ROLE_1;            // *** #1. need to change according to the LVCR role
const LVCR_1_POSITION = LVCR_POSITION_LEFT;    // *** #2. need to change according to the position of Room 1 relative to the Room 2
...
*/
const LVCR_DESIGN_32 = 'lvcr32';
const LVCR_DESIGN_33 = 'lvcr33';
const LVCR_DESIGN_34 = 'lvcr34';

const LVCR_DESIGN_SELF = LVCR_DESIGN_32;
...
const DEVICE_SETTING_C = {
  'lvcr-1': {
    ...
    'presenter_camara_input': '3',
    'presenter_camara_id'   : '2',
    ...
    'video_input_connector': [{'id': 'dummy'}, // Visibility=IfSignal/Never
      ...
      {'id': 3, 'Name':'Presenter Camera', 'CEC_Mode': 'On', 'HDCP_Mode': 'Off', 'InputSourceType': 'camera', 'PreferredResolution': '1920_1080_60', 'PresentationSelection': 'Manual', 'Quality': 'Motion', 'RGBQuantizationRange': '', 'Visibility': 'Never', 'CameraControl': {'CameraId': '2', 'Mode': 'On'}},
      {'id': 4, 'Name':'', 'CEC_Mode': 'On', 'HDCP_Mode': 'Off', 'InputSourceType': 'PC', 'PreferredResolution': '1920_1080_60', 'PresentationSelection': 'OnConnect', 'Quality': 'Sharpness', 'RGBQuantizationRange': '', 'Visibility': 'IfSignal', 'CameraControl': {'CameraId': '', 'Mode': 'Off'}},
      ...
      {'id': 6, 'Name':'', 'CEC_Mode': '', 'HDCP_Mode': '', 'InputSourceType': '', 'PreferredResolution': '', 'PresentationSelection': '', 'Quality': '', 'RGBQuantizationRange': '', 'Visibility': '', 'CameraControl': {'CameraId': '', 'Mode': ''}}],
    ...
    'audio_mic_wireless': [],
    'audio_mic_wireless_lvcr40': [3,4],
    'audio_mic_wireless_lvcr40_combine': [3,4,7,8],
    ...
    'combined_mode': {
      'video_main_source': LVCR_1_POSITION,
      ...
      'audio_input_mic_x': [
        ...
        {'id': 6, 'Mode': 'On', 'Type': MIC_CEILING, 'LocateAt': LVCR_ROLE_2}],
  ...
  'lvcr-2': {
    ...
    'control': {
      ...
      'use_ceiling_speaker': 'always', // never | always | when_combine | when_standalone
      'use_onboard_speaker': 'never', // never | always | when_combine | when_standalone
      'use_video_input_4_only_when_combine': false,
      'monitor_preset_id': PRESET_1,
      'has_sensor': false,
      'state_camera_position': '',
      'state_combined_room': '',
    ...
    'combined_mode': {
      ...
      'video_matrix_assign_x': [
        {'Output': 1, 'SourceID': 4},
        {'Output': 2, 'SourceID': 3},
        {'Output': 3, 'SourceID': 2}],

      'video_matrix_assign_combined_2_4': [],

      'video_matrix_assign_use_video_input_4_only': [
        {'Output': 1, 'SourceID': 4},
        {'Output': 2, 'SourceID': 4},
        {'Output': 3, 'SourceID': 4}],
    },
  ...
  'lvcr-3': {
    ...
    'video_input_connector': [{'id': 'dummy'},
      ...
      {'id': 4, 'Name':'LVCR-1 Output 1', 'CEC_Mode': 'On', 'HDCP_Mode': 'Off', 'InputSourceType': 'Other', 'PreferredResolution': '', 'PresentationSelection': 'Manual', 'Quality': 'Motion', 'RGBQuantizationRange': '', 'Visibility': 'Never', 'CameraControl': {'CameraId': '', 'Mode': 'Off'}},
      {'id': 5, 'Name':'', 'CEC_Mode': 'On', 'HDCP_Mode': 'Off', 'InputSourceType': 'PC', 'PreferredResolution': '', 'PresentationSelection': 'OnConnect', 'Quality': 'Sharpness', 'RGBQuantizationRange': '', 'Visibility': 'IfSignal', 'CameraControl': {'CameraId': '', 'Mode': 'Off'}},
      {'id': 6, 'Name':'Presenter Camera', 'CEC_Mode': 'On', 'HDCP_Mode': 'Off', 'InputSourceType': 'camera', 'PreferredResolution': '', 'PresentationSelection': '', 'Quality': '', 'RGBQuantizationRange': '', 'Visibility': 'Never', 'CameraControl': {'CameraId': '', 'Mode': 'Off'}}],
    ...
    'control': {
      'ui_wireless_mic': 0,
      'use_ceiling_mic': 'always', // never | always | when_combine | when_standalone
      'use_ceiling_speaker': 'never', // never | always | when_combine | when_standalone
      'use_onboard_speaker': 'always', // never | always | when_combine | when_standalone
      'use_video_input_4_only_when_combine': false,
      'monitor_preset_id': PRESET_1,
      'has_sensor': false,
      'state_camera_position': '',
  ...
  'lvcr-4': {
    ...
    // lvcr-4: video_monitor_preset // Output 3 MonitorRole should always First
    'video_monitor_preset': {
      '1': {'output_connecters': [{'id': 1, 'MonitorRole': 'First'}, {'id': 2, 'MonitorRole': 'Second'}, {'id': 3, 'MonitorRole': 'Recorder'}]}, // Video on Left Display. Presentation on Right Display.
      '2': {'output_connecters': [{'id': 1, 'MonitorRole': 'Second'}, {'id': 2, 'MonitorRole': 'First'}, {'id': 3, 'MonitorRole': 'Recorder'}]}, // Presentation on Left Display. Video on Right Display.
      '3': {'output_connecters': [{'id': 1, 'MonitorRole': 'PresentationOnly'}, {'id': 2, 'MonitorRole': 'PresentationOnly'}, {'id': 3, 'MonitorRole': 'Recorder'}]}, // Presentation Only on all Display.
    ...
    // lvcr-4: Non-Room mode related control
    'control': {
      'ui_wireless_mic': 0,
      'use_ceiling_mic': 'always', // never | always | when_combine | when_standalone
      'use_ceiling_speaker': 'never', // never | always | when_combine | when_standalone
    ...
    'combined_mode': {
      ...
      'audio_input_mic': [
        {'id': 1, 'Mode': 'Off', 'Type': MIC_CEILING, 'LocateAt': LVCR_ROLE_4},
        {'id': 2, 'Mode': 'Off', 'Type': MIC_CEILING, 'LocateAt': LVCR_ROLE_4}],
    ...
    // lvcr-4: Standalone behaviour
    'standalone_mode': {
      'audio_input_mic': [
        {'id': 1, 'Mode': 'On', 'Type': MIC_CEILING, 'LocateAt': LVCR_ROLE_4},
        {'id': 2, 'Mode': 'On', 'Type': MIC_CEILING, 'LocateAt': LVCR_ROLE_4}],
...
const cameras = {
  'presenter_track'       : {
    'name'                : 'PresenterTrack',
    'enable'              : 'True',
    'connector'           : DEVICE_SETTING_C[LVCR_ROLE_SELF].presenter_camara_input,
  },
...
async function common_initConfig() {
  ...
  if(setting.control.ui_wireless_mic) {
    setting.audio_mic_wireless = setting.audio_mic_wireless_lvcr40;
  }
...
async function common_setMode(common_setting) {
  ...
  if(state_mic_wireless == C.ON) {
    for(const val of common_setting.audio_input_mic)
      if(!!val.id)  await xconfig(`Audio Input Microphone ${val.id} Mode`, 'Off');
  }
  else {
...
async function common_cbSensorState(event='') {
  ...
  for(const val of gpio_pins) {
    ...
    else if((LVCR_DESIGN_SELF == LVCR_DESIGN_33 || LVCR_DESIGN_SELF == LVCR_DESIGN_34) && val.id == '2' && val.State == sensor_combined)  state_gpio_combine += GPIO_VALUE_PIN_2;
    else if(LVCR_DESIGN_SELF == LVCR_DESIGN_34 && val.id == '3' && val.State == sensor_combined)  state_gpio_combine += GPIO_VALUE_PIN_3;
  }
  setting.control.state_gpio_combine = state_gpio_combine;

  let state_combined_room = '';
  if(LVCR_ROLE_SELF == LVCR_ROLE_1) {
    if(state_gpio_combine & GPIO_VALUE_PIN_1 && state_gpio_combine & GPIO_VALUE_PIN_2 && state_gpio_combine & GPIO_VALUE_PIN_3) state_combined_room = STATE_COMBINED_ROOM_1_2_3_4;
    else if(state_gpio_combine & GPIO_VALUE_PIN_1 && state_gpio_combine & GPIO_VALUE_PIN_2) state_combined_room = STATE_COMBINED_ROOM_1_2_3;
    else if(state_gpio_combine & GPIO_VALUE_PIN_1 && state_gpio_combine & GPIO_VALUE_PIN_3) state_combined_room = STATE_COMBINED_ROOM_1_2_4;
    ...
    if((state_gpio_combine & GPIO_VALUE_PIN_1) && (state_gpio_combine & GPIO_VALUE_PIN_2)) state_combined_room = STATE_COMBINED_ROOM_1_2_4;
    else if(state_gpio_combine & GPIO_VALUE_PIN_1) state_combined_room = STATE_COMBINED_ROOM_1_2;
    else if(state_gpio_combine & GPIO_VALUE_PIN_2) state_combined_room = STATE_COMBINED_ROOM_2_4;

    setting.control.state_combined_room = state_combined_room;
    handleLog('common_cbSensorState', `lvcr_role=${LVCR_ROLE_SELF}, state_gpio_combine=0x${state_gpio_combine.toString(16).padStart(4, "0")}, state_combined_room=${state_combined_room}`);

    if(state_combined_room) await lvcr2_setCombinedMode();
  ...
  else if(LVCR_ROLE_SELF == LVCR_ROLE_4) {
    if(state_gpio_combine & GPIO_VALUE_PIN_1) state_combined_room = STATE_COMBINED_ROOM_2_4;
...
let mic_meters = {};
let camera_score = {'camera_1': 0, 'camera_2': 0, 'camera_tie': 0};
...
async function initVuMeter() {
  ...
  if(B_CAMERA_VIEW_AUTO && LVCR_ROLE_SELF == LVCR_ROLE_1) {
    const lvcr1_mic = device_setting[LVCR_ROLE_1].combined_mode.audio_input_mic_x;
...
//  handleLog('cbVuMeterCheck', `state_combined_room=${state_combined_room}, state_mute=${state_mute}, state_mic_ceiling=${state_mic_ceiling}, state_mic_wireless=${state_mic_wireless}, camera_1_mic_offset=${camera_1_mic_offset}`);
  if(B_CAMERA_VIEW_AUTO && LVCR_ROLE_SELF == LVCR_ROLE_1 && state_combined_room.includes('1_2') && lvcr1_setting.control.ui_camera_view == C.AUTO && setting.control.state_camera_position == C.CAMERA_FRONT) {
    ...
    if(state_mute == C.ON || state_mic_ceiling == C.OFF) {
      if(camera_active_view != 0) {
        xcommand('Video Input SetMainVideoSource', {ConnectorID: setting.combined_mode.video_main_source});
...
//          handleDebug('cbVuMeterCheck', `mic ${mic.id}`, mic);
      ...
      if(camera_1 >= camera_2 + THRESHOLD_MIC_VU) {
        camera_score['camera_1'] = camera_score['camera_1'] + 1;
        camera_score['camera_2'] = 0;
        camera_score['camera_tie'] = 0;
      }
      else if(camera_2 >= camera_1 + THRESHOLD_MIC_VU) {
        camera_score['camera_1'] = 0;
      ...
      handleDebug('cbVuMeterCheck', `active_view=${camera_active_view}, camera_1=${camera_1}, camera_2=${camera_2}`, `score`, camera_score, `camera_1_offset`, camera_1_mic_offset, `mic`, Object.keys(mic_meters));
      if(camera_score['camera_1'] >= THRESHOLD_CAMERA_SWITCH) {
        ...
        if(camera_active_view != 0)  xcommand('Video Input SetMainVideoSource', {ConnectorID: setting.combined_mode.video_main_source});
        camera_active_view = 0;
        camera_score['camera_tie'] = 0;
      }
    }
...
async function lvcr1_initConfig() {
  ...
  let presenter_camera_id = setting.presenter_camara_id;
  let presenter_camera_input = setting.presenter_camara_input;
...
//  handleDebug('lvcr1_setCombinedMode', `mute=${mute}`)
  ...
  setting.combined_mode.audio_input_mic = (STATE_COMBINED_ROOM_1_3 == state_combined_room)? setting.combined_mode.audio_input_mic_1_3 : setting.combined_mode.audio_input_mic_x;

  const audio_output_line = JSON.parse(JSON.stringify(setting.combined_mode.audio_output_line_x));
  let output_line_on = state_combined_room.replace('state_combined_room_', '');
  output_line_on = output_line_on.split('_');
  for(const val of output_line_on)
    audio_output_line[val-1].Mode = 'On';

  setting.combined_mode.audio_output_line = audio_output_line;
  setting.audio_mic_wireless = setting.audio_mic_wireless_lvcr40_combine;
  ...
  if(STATE_COMBINED_ROOM_1_3 == state_combined_room)
    await xcommand('Video Input SetMainVideoSource', {ConnectorID: setting.video_main_source});
  else if(setting.control.ui_camera_view != C.DISABLE) {
...
//  if(!state_combined_room || !call)   return;
  ...
  if(state_combined_room.includes('1_2')) {
    let postion = '';

    switch(ui_camera_view) {
    case C.AUTO:
      await xcommand('Video Input SetMainVideoSource', {ConnectorID: setting.combined_mode.video_main_source});
...
async function lvcr2_setCombinedMode() {
  const setting = device_setting[LVCR_ROLE_SELF];
  const state_combined_room = setting.control.state_combined_room;
  handleLog('lvcr2_setCombinedMode', `state_combined_room=${state_combined_room}`);
  const audio_mic = await xapi.config.get(`Audio Microphones`).catch(e => {return ''});
  const camera = await xapi.config.get('Cameras').catch(e => {return ''});
  handleDebug('lvcr2_setCombinedMode', `audio_mic`, audio_mic);
//  handleDebug('lvcr2_setCombinedMode', `camera`, camera);

  if(!audio_mic || !camera) {
    handleError('lvcr2_setCombinedMode');
    return;
  }

  if(STATE_COMBINED_ROOM_2_4 == state_combined_room) {
    setting.combined_mode.audio_input_mic = JSON.parse(JSON.stringify(setting.combined_mode.audio_input_mic_combined_2_4));
    setting.combined_mode.audio_output_line = JSON.parse(JSON.stringify(setting.combined_mode.audio_output_line_combined_2_4));
    setting.combined_mode.video_matrix_assign = JSON.parse(JSON.stringify(setting.combined_mode.video_matrix_assign_combined_2_4));
  ...
  //    await xcommand('Video Selfview Set', {Mode: 'On'});
    ...
    setting.combined_mode.audio_input_mic = JSON.parse(JSON.stringify(setting.combined_mode.audio_input_mic_x));
    setting.combined_mode.audio_output_line = JSON.parse(JSON.stringify(setting.combined_mode.audio_output_line_x));
    setting.combined_mode.video_matrix_assign = JSON.parse(JSON.stringify(setting.combined_mode.video_matrix_assign_x));
    await common_setCombinedMode();
  }
}
...
async function buttonCombineRoom(value) {
  ...
  case STATE_COMBINED_ROOM_1_2:
    ...
    else if(LVCR_ROLE_SELF == LVCR_ROLE_2)   await lvcr2_setCombinedMode();
    break;
  ...
  case STATE_COMBINED_ROOM_1_2_4:
    setting.control.state_combined_room = value;
    if(LVCR_ROLE_SELF == LVCR_ROLE_1)   await lvcr1_setCombinedMode();
    else if(LVCR_ROLE_SELF == LVCR_ROLE_2)   await lvcr2_setCombinedMode();
    else if(LVCR_ROLE_SELF == LVCR_ROLE_4)   await common_setCombinedMode();
    break;

  case STATE_COMBINED_ROOM_1_2_3_4:
    setting.control.state_combined_room = value;
    if(LVCR_ROLE_SELF == LVCR_ROLE_1)   await lvcr1_setCombinedMode();
    else if(LVCR_ROLE_SELF == LVCR_ROLE_2)   await lvcr2_setCombinedMode();
    else await common_setCombinedMode();
    break;

  case STATE_COMBINED_ROOM_2_4:
    setting.control.state_combined_room = value;
    if(LVCR_ROLE_SELF == LVCR_ROLE_2)   await lvcr2_setCombinedMode();
    else if(LVCR_ROLE_SELF == LVCR_ROLE_4)   await common_setCombinedMode();
    break;
  }
...
async function setCameraTrack(camera_track, camera) {
  ...
  if(camera_track == C.CAMERA_FRONT) {
    ...
    await xcommand('Cameras SpeakerTrack Activate');
    if(LVCR_ROLE_SELF == LVCR_ROLE_1 && state_combined_room && STATE_COMBINED_ROOM_1_3 != state_combined_room && setting.control.ui_camera_view != C.DISABLE)
      await lvcr1_setCameraView();
...
async function setMicMode(id, value) {
  ...
  if(id == 1) {
    ...
    let audio_input_mic = [];
    let b_standalone = (!state_combined_room || state_combined_room == STATE_COMBINED_ROOM_1_3)? true : false;
    if(b_standalone)  audio_input_mic = setting.standalone_mode.audio_input_mic;
    else audio_input_mic = setting.combined_mode.audio_input_mic_x;
    ...
    if(value == C.ON) {
      ...
      for(const val of setting.combined_mode.audio_input_mic_x)
...
async function delayed_updateUI() {
  ...
  if(LVCR_ROLE_SELF == LVCR_ROLE_1 || LVCR_ROLE_SELF == LVCR_ROLE_2) {
...
//      await setWidgetValue(widgetIds.mic_1_level, input_mic[0].Level);
      ...
      if(LVCR_ROLE_SELF == LVCR_ROLE_1 && setting.control.state_combined_room && setting.control.state_combined_room != STATE_COMBINED_ROOM_1_3 && lvcr2_setting.control.ui_wireless_mic) {
        for(let i=0; i<lvcr2_setting.control.ui_wireless_mic; i++) {
          let mic_id = setting.audio_mic_wireless[i+2];
          await setWidgetValue(`toggle-mic_${mic_id}_mode`, input_mic[mic_id-1].Mode);
          await setWidgetValue(`spinner-mic_${mic_id}_level`, input_mic[mic_id-1].Level);
        }
      }
...
async function rebuildUI() {
  ...
  if(SHOW_PAGE_AUDIO) {
    let row_mic_wirless = '';
    let ui_id = (LVCR_ROLE_SELF == LVCR_ROLE_1) ? 1 : 3;
  ...
  if(SHOW_PAGE_CAMERA) {
    const row_camera_view = (LVCR_ROLE_SELF == LVCR_ROLE_1 && state_combined_room.includes('1_2') && setting.control.ui_camera_view != C.DISABLE) ? ROW_CAMERA_VIEW : '';
  ...
  if((LVCR_ROLE_SELF == LVCR_ROLE_1 || LVCR_ROLE_SELF == LVCR_ROLE_2) && !page_control) {
...
async function listenOn() {
  ...
  if(LVCR_DESIGN_SELF == LVCR_DESIGN_33 || LVCR_DESIGN_SELF == LVCR_DESIGN_34)  await xapi.status.on(`GPIO Pin 2`, (event) => common_cbSensorState(event));
  if(LVCR_DESIGN_SELF == LVCR_DESIGN_34)  await xapi.status.on(`GPIO Pin 3`, (event) => common_cbSensorState(event));
...
<Row>
  ...
  <Widget>
    ...
    <ValueSpace>
      ...
      <Value>
        <Key>${STATE_COMBINED_ROOM_1_2_4}</Key>
        <Name>1+2+4</Name>
      </Value>
      <Value>
        <Key>${STATE_COMBINED_ROOM_1_2_3_4}</Key>
        <Name>1+2+3+4</Name>
      </Value>
      <Value>
        <Key>${STATE_COMBINED_ROOM_2_4}</Key>
        <Name>2+4</Name>
      </Value>
    </ValueSpace>
  </Widget>
