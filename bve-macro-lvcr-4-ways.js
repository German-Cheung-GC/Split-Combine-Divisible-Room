...
*/
const LVCR_DESIGN_40 = 'lvcr40';
const LVCR_DESIGN_41 = 'lvcr41';

const LVCR_DESIGN_SELF = LVCR_DESIGN_40;
...
async function common_cbSensorState(event='') {
  ...
  for(const val of gpio_pins) {
    ...
    else if(val.id == '2' && val.State == sensor_combined)  state_gpio_combine += GPIO_VALUE_PIN_2;
    else if(val.id == '3' && val.State == sensor_combined)  state_gpio_combine += GPIO_VALUE_PIN_3;
  }
...
async function listenOn() {
  ...
  await xapi.status.on(`GPIO Pin 2`, (event) => common_cbSensorState(event));
  await xapi.status.on(`GPIO Pin 3`, (event) => common_cbSensorState(event));
